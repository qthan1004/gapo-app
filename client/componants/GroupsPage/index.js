$(document).ready(() => {
            ...
            if (!data.isSuccess) {
                ...
                data.data.forEach(group => {
                    if (group._id === groupID) {
                        $('.logogroup').html(` <span>${group.name}</span>`)
                        userdata.forEach(mem => {
                            if (group.memberID.indexOf(mem._id) != -1) {
                                $('#list-user').append(`<li class="item-r">
                            <a href="#"><i class="fa fa-user-circle" aria-hidden="true"></i><span
                                    class="md-2">${mem.firstname} ${mem.lastname}</span></a>
                            </li>`)
                            }
                        })
                    }
    ...
    function getAllGroup() {
        $.ajax({
            ...
            if (!data.isSuccess) {
                ...
                data.data.forEach(group => {
                    $('.group').append(
                        `<li class="item-r group-item" key = '${group._id}'>
    ...
    //CLICK ON GROUP
    $('.row').on('click', " .bg-white .navbar .navbar-collapse .group .group-item ", (event) => {
        event.preventDefault()
        let groupID = $(event.currentTarget).attr('key')
        localStorage.setItem('GroupID', groupID)
        window.location.replace('http://127.0.0.1:5500/gapo%20app/client/componants/GroupsPage/index.html')
    })
        ...
        $.ajax({
            ...
            if (!data.isSuccess) {
                ...
                data.posts.forEach(post => {
                    if (post.groupID.length != 0 && post.groupID === groupID) {
                        $('#content').prepend(`
    ...
    $('.btn-post').click(() => {
        ...
        $.ajax({
            url: `http://localhost:3000/posts/${groupID}`,
            data: JSON.stringify({
                author,
                content,
                groupID
